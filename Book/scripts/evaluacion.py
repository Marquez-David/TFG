...
def mostrar_tabla_metricas():
    ...
    tabla_metricas = PrettyTable(['N. del paquete', 'Accuracy', 'Precision', 'Recall', 'F1'])

    '''
    #ejecucion de todos los algoritmos de extraccion
    for path in Path('algoritmos_extraccion').glob('*.py'):
        exec(open(path).read())
    '''
        
    #carga de json y evaluacion de algoritmos
...
'''
def calcular_accuracy(texto_base, texto_extraido):
